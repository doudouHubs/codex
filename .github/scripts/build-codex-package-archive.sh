...
codex_supervisor_bin_provided="false"
...
while [[ $# -gt 0 ]]; do
  case "$1" in
    ...
    --codex-supervisor-bin)
      resource_args+=(--codex-supervisor-bin "${2:?--codex-supervisor-bin requires a value}")
      codex_supervisor_bin_provided="true"
      shift 2
      ;;
    --rg-bin)
...
codex_supervisor_bin="${entrypoint_dir%/}/codex-supervisor${exe_suffix}"
if [[ "$codex_supervisor_bin_provided" == "false" && -f "$codex_supervisor_bin" ]]; then
  resource_args+=(--codex-supervisor-bin "$codex_supervisor_bin")
fi
