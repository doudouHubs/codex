...
4. Automated test runs must use no more than two build jobs and two concurrent test threads. Use `--build-jobs 2 --test-threads 2` with `just test`; do not raise either limit unless the user explicitly requests it.
