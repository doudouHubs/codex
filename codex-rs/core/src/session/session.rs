...
impl Session {
    ...
    pub(crate) async fn new(
        ...
        let auth_and_mcp_fut = async move {
            ...
            let mcp_projection = if mcp_runtime_mode == McpRuntimeMode::Disabled {
                // Prompt/side 线程启动时直接使用空投影，避免读取 MCP 配置、插件能力或扩展 overlay。
                crate::mcp::empty_mcp_runtime_projection(&config_for_mcp)
            } else {
                mcp_manager_for_mcp
        ...
        let session_result: anyhow::Result<Arc<Self>> = async {
            ...
            let plugin_skill_warmup = async {
                ...
                {
                    // Prompt 线程按需读取文件，不预热项目指令、skills 或 plugins。
                    Vec::new()
                } else {
