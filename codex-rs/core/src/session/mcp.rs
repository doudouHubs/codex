...
impl Session {
    pub(crate) async fn runtime_mcp_config(&self, config: &Config) -> McpConfig {
        if self.mcp_runtime_mode == McpRuntimeMode::Disabled {
            return crate::mcp::empty_mcp_runtime_projection(config).config;
        }
    ...
    pub(crate) async fn refresh_mcp_if_dirty(self: &Arc<Self>) {
        if self.mcp_runtime_mode == McpRuntimeMode::Disabled {
            // Prompt/side 线程使用启动时创建的空 runtime，禁止 dirty 状态机再次加载 MCP。
            return;
        }
    ...
    pub(crate) async fn mcp_runtime_for_step(
        ...
        if self.mcp_runtime_mode == McpRuntimeMode::Disabled {
            // 禁用线程直接返回空 binding，既不刷新，也不根据当前 turn 的环境重建 MCP。
            return Arc::new(codex_mcp::McpBinding::empty(Arc::new(
                crate::mcp::empty_mcp_runtime_projection(&turn_context.config).config,
            )));
        }
