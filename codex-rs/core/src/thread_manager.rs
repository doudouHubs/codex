...
struct ThreadSpawnRequest {
    ...
    // Runtime 模式必须随排队请求传递，避免异步 spawn 过程中回退到默认的完整 MCP runtime。
...
impl ThreadSpawnRequest {
    ...
    fn with_runtime_modes(
        mut self,
        mcp_runtime_mode: McpRuntimeMode,
        thread_runtime_mode: ThreadRuntimeMode,
    ) -> Self {
        self.mcp_runtime_mode = mcp_runtime_mode;
        self.thread_runtime_mode = thread_runtime_mode;
        self
    }
}
...
impl ThreadManager {
    ...
    async fn start_thread_with_options_and_modes(
        ...
        request = request.with_runtime_modes(mcp_runtime_mode, thread_runtime_mode);
    ...
    pub async fn fork_prepared_thread(
        ...
        self.fork_prepared_thread_with_modes(
            config,
            prepared,
            thread_source,
            parent_trace,
    ...
    /// Fork prepared history without exposing MCP capabilities to the child runtime.
    pub async fn fork_prepared_thread_without_mcp(
        &self,
        config: Config,
        prepared: PreparedFork,
        thread_source: Option<ThreadSource>,
    ...
    /// Fork prepared history into the isolated prompt-optimization runtime.
    pub async fn fork_prepared_thread_for_prompt(
        &self,
        config: Config,
        prepared: PreparedFork,
        thread_source: Option<ThreadSource>,
    ...
    async fn fork_prepared_thread_with_modes(
        &self,
        config: Config,
        prepared: PreparedFork,
        thread_source: Option<ThreadSource>,
